...
const WomenProductList: React.FC = () => {
  ...
  const filterByPrice = (product: Product) => {
    if (priceFilter === 'below200') return product.price < 200;
    if (priceFilter === '200to400') return product.price >= 200 && product.price <= 400;
    if (priceFilter === 'above400') return product.price > 400;
    return true;
  };

  const womenProducts = products
    .filter((product) => product.cat?.toLowerCase() === 'women')
              ...
              <div key={product._id} className="cart-card">
                ...
                <div className="card-buttons">
                  ...
                  <span
                    ...
                    style={{ color: isWishlisted(product._id) ? 'red' : 'gray', fontSize: '20px', cursor: 'pointer', marginLeft: '10px' }}
                  >
